...
@interface TTCosmeticPanelView()<UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout>
{
    //上一次选中的section
    ...
    TTCosmeticGroup *_cosmeticGroup;
    
    //美妆初始化
    BOOL _cosmeticEffectInit;
}
...
@implementation TTCosmeticPanelView

#pragma mark - instance
+ (instancetype)beautyPanelWithFrame:(CGRect)frame beautyTarget:(id<TTBeautyProtocol>)beautyTarget
{
    TTCosmeticPanelView *view = [[TTCosmeticPanelView alloc] initWithFrame:frame beautyTarget:beautyTarget];
    return view;
}
...
{
    _cosmeticGroup = [[TTCosmeticGroup alloc] init];
    
    self.cosmeticDataSet = [NSMutableArray array];
    self.stickerDataSet = [NSMutableArray array];
    NSArray *cosmeticArray = [TuBeautyPanelConfig cosmeticDataSet];
    ...
    [_collectionView registerClass:[TuCosmeticPanelViewCell class] forCellWithReuseIdentifier:@"TTCosmeticPanelViewCell"];
}
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section
{
    ...
    //从配置中获取元素数量
    NSArray *dataSource = [TuBeautyPanelConfig dataSetWithCosmeticCode:data.cosmeticCode];
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.item == 0)
    {
        NSArray *dataSet = [TuBeautyPanelConfig cosmeticDataSet];
...
{
    TuCosmeticPanelViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"TTCosmeticPanelViewCell" forIndexPath:indexPath];
...
{
    if (!_cosmeticEffectInit) {
        if ([self.beautyTarget respondsToSelector:@selector(addEffect:)]) {
            [self.beautyTarget addEffect:TTEffectTypeCosmetic];
        }
        _cosmeticEffectInit = YES;
    }
    
    TuCosmeticPanelViewCell *cell = (TuCosmeticPanelViewCell *)[collectionView cellForItemAtIndexPath:indexPath];
    ...
    if (indexPath.section == 0)
    {
        ...
        if (count != 0)
        {
            if (_delegate && [_delegate respondsToSelector:@selector(cosmeticPanelView:code:enable:)])
            {
                [_delegate cosmeticPanelView:self code:@"cosmeticReset" enable:NO];
                _cosmeticEffectInit = NO;
            }
        }
    }
    ...
    else if (indexPath.section == 2)
    {
        ...
        if (indexPath.item == 0)
        {
            ...
            if (data.cosmeticSelectType == TuCosmeticSelectTypeSelected)
            {
                ...
                if (self.delegate && [self.delegate respondsToSelector:@selector(cosmeticPanelView:changeCosmeticType:)])
                {
                    [self.delegate cosmeticPanelView:self changeCosmeticType:data.cosmeticCode];
                }
            }
        }
        else if (indexPath.item == 2)
        {
            ...
            if (_lipStickCode.length != 0)
            {
                ...
                if (self.delegate && [self.delegate respondsToSelector:@selector(cosmeticPanelView:didSelectedLipStickType:stickerName:)])
                {
                    [self.delegate cosmeticPanelView:self didSelectedLipStickType:_lipStickData.lipStickType stickerName:itemData.stickerName];
                }
            }
        }
        else
        {
            ...
            else if ([stickerData.stickerName isEqualToString:@"reset"])
            {
                ...
                if ([self.selectDataSet[indexPath.section] integerValue] != 0)
                {
                    ...
                    TuCosmeticPanelViewCell *lastCell = (TuCosmeticPanelViewCell *)[collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:selectItem inSection:indexPath.section]];
                    ...
                    if (_delegate && [_delegate respondsToSelector:@selector(cosmeticPanelView:code:enable:)])
                    {
                        [_delegate cosmeticPanelView:self code:data.cosmeticCode enable:NO];
                        _lipStickCode = nil;
                    }
    ...
    else if (indexPath.section == 4)
    {
        ...
        else if (indexPath.item == 3)
        {
            ...
            if (_eyebrowCode.length != 0)
            {
                ...
                NSString *stickerCode = [TuBeautyPanelConfig eyeBrowCodeByBrowType:_eyeBrowData.eyeBrowType stickerName:stickerData.stickerName];
                if (stickerCode.length == 0)
                {
                    return;
                }
                
                if (_delegate && [_delegate respondsToSelector:@selector(cosmeticPanelView:code:value:)])
                {
                    _eyebrowCode = stickerCode;
                    [self.delegate cosmeticPanelView:self code:data.cosmeticCode value:[stickerCode integerValue]];
                }
            }
        }
        else
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                NSString *stickerCode = [TuBeautyPanelConfig effectCodeByCosmeticCode:data.cosmeticCode stickerName:stickerData.stickerName];
                if (stickerCode.length == 0)
                {
                    return;
                }
...
- (void)judgeSliderBarIsHidden:(BOOL)hidden section:(NSInteger)section
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(cosmeticPanelView:closeSliderBar:)])
    {
        [self.delegate cosmeticPanelView:self closeSliderBar:hidden];
    }
}
...
{
    TuCosmeticPanelViewCell *lastCell = (TuCosmeticPanelViewCell *)[_collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:0 inSection:_lastSelectSection]];
    //将上一次选中的section收起
...
- (void)setResetCosmetic:(BOOL)resetCosmetic
{
    ...
    if (_resetCosmetic)
    {
        ...
        for (int i = 0; i < [TuBeautyPanelConfig cosmeticDataSet].count; i++)
        {
...
{
    NSArray *dataSource = [TuBeautyPanelConfig dataSetWithCosmeticCode:cosmeticCode];
