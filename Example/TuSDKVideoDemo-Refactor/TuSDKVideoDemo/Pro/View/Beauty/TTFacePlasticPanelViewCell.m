...
@implementation TTFacePlasticPanelViewCell
...
- (void)setBeautyItem:(TTFacePlasticItem *)beautyItem
{
    _pointView.hidden = _selectImageView.hidden = YES;
    _thumbnailView.hidden = NO;
    
    //间隔符
    if ([beautyItem.code isEqualToString:@"point"]) {
        ...
        _titleLabel.text = beautyItem.name;
        _thumbnailView.image = beautyItem.icon;
        _selectImageView.image = beautyItem.selectIcon;
        
    }
    
    if (!beautyItem.isSelected) {
        ...
        //选中时显示选中视图
        if (![beautyItem.code isEqualToString:@"reset"]) {
