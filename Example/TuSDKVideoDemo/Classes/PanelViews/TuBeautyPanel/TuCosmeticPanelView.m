...
- (void)initWithSubViews
{
    ...
    [_collectionView registerClass:[TuCosmeticPanelViewCell class] forCellWithReuseIdentifier:@"TuCosmeticCell"];
}
...
{
    TuCosmeticPanelViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"TuCosmeticCell" forIndexPath:indexPath];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0)
    {
        ...
        if (count != 0)
        {
            if (_delegate && [_delegate respondsToSelector:@selector(tuCosmeticPanelView:paramCode:enable:)])
            {
                [_delegate tuCosmeticPanelView:self paramCode:@"cosmeticReset" enable:NO];
            }
        }
    }
        ...
        else
        {
            ...
            else if ([stickerData.stickerName isEqualToString:@"reset"])
            {
                ...
                if ([self.selectDataSet[indexPath.section] integerValue] != 0)
                {
                    ...
                    if (_delegate && [_delegate respondsToSelector:@selector(tuCosmeticPanelView:paramCode:enable:)])
                    {
                        [_delegate tuCosmeticPanelView:self paramCode:data.cosmeticCode enable:NO];
                        _lipStickCode = nil;
                    }
    ...
    else if (indexPath.section == 4)
    {
        ...
        else if (indexPath.item == 3)
        {
            ...
            if (_eyebrowCode.length != 0)
            {
                ...
                if (_delegate && [_delegate respondsToSelector:@selector(tuCosmeticPanelView:paramCode:value:)])
                {
                    _eyebrowCode = stickerCode;
                    [self.delegate tuCosmeticPanelView:self paramCode:data.cosmeticCode value:[stickerCode integerValue]];
                }
            }
        }
        else
